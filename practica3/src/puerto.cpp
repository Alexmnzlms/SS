...
void fin_atraque(){
	...
	if (tormenta == false){
		if (encola_sal > 0){
	 	...
	 	else if ((atraques_libres > 0) && (encola_lleg > 0)){ //el remolcador va por un barco a la bocana
			nodo.suceso = SUCESO_FIN_VIAJE_AT_BO;
			nodo.tiempo = reloj+tviajevacio;
			nodo.reg_cola = reg_cola_null;
			insertar_lsuc(nodo);
...
void fin_desatraque(){
	...
	if ((encola_lleg > 0) && (tormenta == false)){ //comienza a atracar un barco
		...
		nodo.reg_cola = reg_cola;
		insertar_lsuc(nodo);
	} else { //hay tormenta o no hay barcos esperando: el remolcador se vuelve a los puntos de atraque
...
void fin_viaje_bo_at(){
	...
	if (tormenta == false){
		if (encola_sal > 0){ //comienza a desatracar un barco
	 	...
	 	else if ((atraques_libres > 0) && (encola_lleg > 0)){ //el remolcador va por un barco
			nodo.suceso = SUCESO_FIN_VIAJE_AT_BO;
			nodo.tiempo = reloj+tviajevacio;
			nodo.reg_cola = reg_cola_null;
			insertar_lsuc(nodo);
